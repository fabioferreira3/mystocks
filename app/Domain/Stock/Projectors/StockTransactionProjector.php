...
class StockTransactionProjector extends Projector {
    ...
    public function onStockTransactionCreated(StockTransactionCreated $event)
    {
        ...
        if (!StockPosition::byStockId($transactionData['stock_id']) && isset($transactionData['added'])) {
            return StockPosition::createWithAttributes([
                'stock_id' => $transactionData['stock_id'],
                'position' => $transactionData['added'],
                'current_invested_value' => $transactionData['added'] * $transactionData['unit_price'] + $transactionData['taxes'],
